...
class Dispatcher:
    ...
    def request_buy_or_sell(self, instrument, volume, price, buy=True, callback=None):
        ...
        print("instrument:", self.algotrader.tickers_numbers.keys()[self.algotrader.tickers_numbers.values().index(instrument)])
        print("volume:",volume)
        print("price:",price)
        if callback:
    ...
    def backtest_algotrader(self, df_45_by_10min: pd.DataFrame, news_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for timepoint in timepoints_df_45_by_10min:
            ...
            self.algotrader.timer_tic(time=datetime.datetime.utcfromtimestamp(timepoint.tolist()/1e9), cur_ticker_open_prices=cur_ticker_open_prices, prediction=predictions)
...
class Algotrader:
    ...
    def update_strategy(self, predictions, cur_ticker_open_prices):
        self.requests_prices = {}
        self.volumes_to_buy = {}
        self.expected_income = []
        self.min_relative_income = []
    ...
    def timer_tic(self, time, cur_ticker_open_prices, prediction):
        ...
        if t - self.opening_time < self.opening_delta:
            ...
            self.update_strategy(prediction, cur_ticker_open_prices)
